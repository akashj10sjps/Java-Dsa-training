...
class Solution {
    ...
    private void solve(int[] nums,List<Integer> output,
        ...
        if(index>=nums.length){
            ans.add(new ArrayList<>(output));//Add a copy of output
            return;
        }
        //Exclude case->Skip the current index and move a step
        solve(nums,output,index+1,ans);
        //Include case-> Take the current element and move a step
